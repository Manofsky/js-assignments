...
function doRectanglesOverlap(rect1, rect2) {
  ...
  return !(rect1.left > right2 || rect2.left > right1 || rect1.top > bottom2 || rect2.top > bottom1);
}
...
function isInsideCircle(circle, point) {
  ...
  return (circle.radius > Math.hypot(cathetX, cathetY));
}
...
function isBracketsBalanced(str) {
  ...
  str.split('').forEach((value) => {
    if (Object.values(bracketPairs).includes(value)) {
      stack.push(value);
    } else {
      if (stack.length === 0) isBalanced = false;
      if (bracketPairs[value] === stack.at(-1)) {
        stack.pop();
